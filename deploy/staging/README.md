...
The Southeast Blueprint Explorer and Midwest Blueprint Explorer follow virtually
identical procedures for setup and maintenance. The Southeast Species Status
Landscape Assessment Tool leverages data shared with the Southeast Blueprint and
follows similar but non-identical procedures.
...
```bash
mkdir /var/www/test-midwestblueprint
...
PUBLIC_GOOGLE_ANALYTICS_ID=<id>
PUBLIC_CONTACT_EMAIL=<contact email>
PUBLIC_CONTACT_URL=<staff page URL>
PUBLIC_BLUEPRINT_URL=<Blueprint page URL>
...
These are also placed in the `/data/se` folder:
...
Repeat this process for the `mli-blueprint` project and place these in the
Midwest Blueprint directory for that year's data release on FileShare:

- `data/inputs` (may need to be broken into multiple files due to size)
- `data/results`

Extract these to the `/data/midwest` folder:
...
for the item in USFWS FileShare and copy the Direct Link at the bottom.
...
set -a
source .env
docker compose restart midwest-worker midwest-api
```

Then verify the services came up properly:

```bash
docker compose logs --tail midwest-worker
docker compose logs --tail midwest-api
```
...
- https://apps.fws.gov/test-midwestblueprint
